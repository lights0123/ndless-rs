...
impl KeypadListener {
	...
	pub(crate) fn poll(&self) {
		...
		iter_keys()
			.for_each(|key| {
				if let Some((i, _)) = keys.iter().enumerate().find(|(_, other)| key == **other) {
					...
					change = true;
					keys.push(key);
					queues.iter_mut().for_each(|queue| queue.queue.push(KeyEvent { key, state: KeyState::Pressed }).ignore());
		...
		for _ in retain_i..keys.len() {
			...
			queues.iter_mut().for_each(|queue| queue.queue.push(KeyEvent { key, state: KeyState::Released }).ignore());
		}
		if change {
