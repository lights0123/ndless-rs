...
mod tests {
	...
	fn seek_past_i64() {
		...
		assert_eq!(
			r.seek(SeekFrom::Current(0x7fff_ffff_ffff_fff0)).unwrap(),
			0x7fff_ffff_ffff_fff6
		);
		assert_eq!(
			r.seek(SeekFrom::Current(0x10)).unwrap(),
			0x8000_0000_0000_0006
		);
		assert_eq!(r.seek(SeekFrom::Current(0)).unwrap(), 0x8000_0000_0000_0006);
		assert!(r.seek(SeekFrom::Current(0x7fff_ffff_ffff_fffd)).is_err());
		assert_eq!(
			r.seek(SeekFrom::Current(-0x8000_0000_0000_0000)).unwrap(),
			6
		);
